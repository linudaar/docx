...
func (m *MyCharData) UnmarshalXMLEx(d *Decoder, start StartElement) error {
	for {
		t, err := d.Token()
...
func (m *MyCharData) UnmarshalXMLAttrEx(attr Attr) error {
	panic("must not call")
}
...
func (m *MyAttr) UnmarshalXMLAttrEx(attr Attr) error {
	m.attr = attr.Value
	return nil
}
